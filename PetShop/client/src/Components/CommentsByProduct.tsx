...
const ratingOptions = [
  ...
  const handleDeleteComment = async (commentId: string) => {
    try {
      ...
      const res = await authApi(user?.tokenInfo.accessToken).delete(endpoints['deleteComment'](commentId))
      if (res.status === 204) {
        // console.log(res)
            ...
            <Card sx={{ backgroundColor: '#fefefe', borderRadius: 3, boxShadow: 4 }}>
              ...
              {user && user._id === comment.user._id && (
                <Stack direction="row" spacing={2} justifyContent="flex-end" mb={2}>
                  ...
                  <Button variant="outlined" color="info" onClick={() => navigate(`/comments/${comment._id}/update`,{ state: locaion.pathname })}>
                    Update
                  </Button>
                </Stack>
              )}
